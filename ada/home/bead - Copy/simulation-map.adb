...
protected body Map is
        ...
        for I in Coord'First..Coord'Last/2 loop
            for J in Coord'Range loop
                Sum := Sum + 1;
                Space(I,J).Ship := new Ship(new String'("Imp" & Integer'Image(Sum)), Imperial); 
            end loop;
        end loop;
        ...
        for I in Coord'Last/2+1..Coord'Last loop
            for J in Coord'Range loop
                Sum := Sum + 1;
                Space(I,J).Ship := new Ship(new String'("Reb" & Integer'Image(Sum)), Rebel);
            end loop;
        end loop;
                ...
                for I in Coord'First..Coord'Last loop
                    for J in Coord'Range loop
                        if Space(I,J).Alive then
                            declare
                                AA : Agent_Access := new Agent(Space(I,J).Ship);
                            begin
                                --AA.Chtulhu;
                                null;
                            end;
